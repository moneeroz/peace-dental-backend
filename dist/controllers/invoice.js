...
export const filteredInvoices = async (req, res) => {
    ...
    try {
        const data = await db
            ...
            .where(or(ilike(patient.name, `%${query}%`), ilike(doctor.name, `%${query}%`), ilike(invoice.reason, `%${query}%`)))
...
export const fetchPatientInvoices = async (req, res) => {
    const { id } = req.params;
    try {
        const data = await db
            .select({
...
// Get invoices pages
export const fetchInvoicesPages = async (req, res) => {
    ...
    try {
        const pages = await db
            ...
            .from(invoice)
            .where(and(or(ilike(patient.name, `%${query}%`), ilike(patient.phone, `%${query}%`), ilike(doctor.name, `%${query}%`), ilike(invoice.reason, `%${query}%`))))
            .leftJoin(patient, eq(invoice.patientId, patient.id))
            .leftJoin(doctor, eq(invoice.doctorId, doctor.id));
