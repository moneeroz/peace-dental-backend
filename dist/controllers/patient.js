...
export const filteredPatients = async (req, res) => {
    ...
    try {
        const data = await db
            ...
            .where(or(ilike(patient.name, `%${query}%`), ilike(patient.phone, `%${query}%`)))
...
// Get patients pages
export const fetchPatientsPages = async (req, res) => {
    ...
    try {
        const pages = await db
            ...
            .from(patient)
            .where(and(or(ilike(patient.name, `%${query}%`), ilike(patient.phone, `%${query}%`))));
