...
export const filteredAppointments = async (req: Request, res: Response) => {
      ...
      .where(
        and(
          or(
            ...
            sql`appointment.date::text ILIKE ${`%${query}%`}`,
            ilike(appointment.reason, `%${query}%`),
          ),
        ),
      )
...
export const fetchAppointmentsPages = async (req: Request, res: Response) => {
